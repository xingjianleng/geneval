...
def load_file(h5f, filename):
    if filename.endswith(".png") or filename.endswith(".jpg"):
        return Image.open(io.BytesIO(np.array(h5f[filename])))
    elif filename.endswith(".jsonl"):
        return json.loads(np.array(h5f[filename]).tobytes().decode('utf-8'))
    else:
...
def parse_args():
    ...
    parser.add_argument("--max-prompts", type=int, default=553,
                        help="Maximum number of prompts of the dataset, in case DDP generates more than this number.")
    # Other arguments
...
def evaluate_image(h5f, filepath, metadata):
    img_pil = load_file(h5f, filepath)
    img_arr = np.array(img_pil)[:,:,::-1]
    result = inference_detector(object_detector, img_arr)
    bbox = result[0] if isinstance(result, tuple) else result
    segm = result[1] if isinstance(result, tuple) and len(result) > 1 else None
    image = ImageOps.exif_transpose(img_pil)
    detected = {}
...
def main(args):
    ...
    filenames = get_all_datasets(h5f)
    indices = [str(i).zfill(5) for i in range(args.max_prompts)]
    png_names = sorted(
        [
            elem
            for elem in filenames
            if elem.endswith(".png") and elem.split("/")[0] in indices
        ]
    )
    metadata_names = sorted(
        [
            elem
            for elem in filenames
            if elem.endswith("metadata.jsonl") and elem.split("/")[0] in indices
        ]
    )

    for metadata_name in tqdm(metadata_names):
        metadata = load_file(h5f, metadata_name)
        curr_sample_dir = os.path.join(os.path.dirname(metadata_name), "samples")
        curr_samples = [
            elem
            for elem in png_names
            if elem.startswith(curr_sample_dir)
        ]
        for png_name in curr_samples:
            result = evaluate_image(h5f, png_name, metadata)
